...
const ROOT_SETLIST       = path.join(__dirname, 'public', 'Vinyl Setlist');
const BG_SETLIST         = path.join(__dirname, 'public', 'Vinyl Setlist Background');
const BG_Vinyl           = path.join(__dirname, 'public', 'Vinyl Background');
const BG_LYRICS          = path.join(__dirname, 'public', 'Vinyl Lyrics Background');
const ROOT_USER_SETLIST  = path.join(__dirname, 'public', 'Setlist');

// Ensure the folders exist
[ROOT_SETLIST, BG_SETLIST, BG_Vinyl, BG_LYRICS, ROOT_USER_SETLIST].forEach(folder => {
...
function buildArtistLyrics(artist) {
   return [normalizeForAzLyrics(artist)];
}
...
app.get('/background/Vinyl', (req, res) => { res.json(getImageFiles(BG_Vinyl)); });
...
async function downloadImage(url, destPath) {
  const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (Vinyl Deck)' } });
...
async function downloadLyricsHtml(mp3Path) {
  const ROOT_SETLIST = path.join('C:\\Users\\krisk\\Downloads\\Vinyl\\public', 'Vinyl Setlist');
  ...
  if (!acceptedSource) {
    fs.writeFileSync(canonicalTxt, 'Not found.', 'utf8');
...
function scrapeLyricsTxt(htmlFile, mp3Path) {
  ...
  if (!fs.existsSync(htmlFile)) {
    fs.writeFileSync(targetFile, 'Not found.', 'utf8');
    return { txtFile: targetFile, scraped: true };
  }

  const html = fs.readFileSync(htmlFile, 'utf8');
  const lyricsText = extractLyrics(html) || 'Not found.';
...
app.post('/api/setlists/append', express.json(), (req, res) => {
  ...
  try {
    // Normalize to relative under Vinyl Setlist/
    ...
    if (idx >= 0) {
      relPath = relPath.substring(idx); // keep "Vinyl Setlist/Artist/Album/Track.mp3"
    } else {
      // fallback: prepend if missing
      relPath = 'Vinyl Setlist/' + relPath.replace(/^\/+/, '');
    }
...
app.post('/api/setlists/deleteSetlistTrack', express.json(), async (req, res) => {
  try {
    ...
    const norm = (s) => s.replace(/\\/g, '/')
                         .replace(/^Vinyl Setlist\/+/i, '') // strip prefix
                         .trim();
...
const ROOT = path.join(__dirname, 'public', 'Vinyl Setlist');
