(() => {
  ...
  async function preloadDurations(relSetlist) {
    ...
    for (let i = 0; i < relSetlist.length; i++) {
      const url = '/Vinyl Setlist/' + relSetlist[i];
      durations[i] = 0;
  ...
  function renderSetlistCard(titleBase) {
    ...
    img.onerror = () => { img.src = '/Vinyl Setlist Background/default.jpg'; };
  ...
  // ---------- Helpers to enable or disable Vinyl.js ----------
  function disableVinylJS(audio) {
    if (!audio) return;
    audio.onended = null;
    audio.removeEventListener('ended', window.VinylHandleEnded);
    audio.removeEventListener('play', window.VinylHandlePlay);
  }

  function enableVinylJS(audio) {
    if (!audio) return;
    if (typeof window.VinylHandleEnded === 'function') {
      audio.addEventListener('ended', window.VinylHandleEnded);
    }
    if (typeof window.VinylHandlePlay === 'function') {
      audio.addEventListener('play', window.VinylHandlePlay);
    }
  }
  ...
  window.disableVinylJS = disableVinylJS;
  window.enableVinylJS = enableVinylJS;
  window.renderSetlistCard = renderSetlistCard;

// ---------- Core: Load setlist ----------
async function loadSetlistIntoVinyl(selectedFile) {
  ...
  if (coverImg) {
    ArtistAlbumCover(baseTitle, null, true).then(src => {
  coverImg.src = src;
});
  ...
  if (audio) {
    ...
    if (window.playlist.length) {
      audio.src = '/Vinyl Setlist/' + window.playlist[0];
      audio.load();
    }
...
if (carousel) {
  ...
  card.addEventListener('click', async () => {
    await loadSetlistIntoVinyl(selectedFile);
...
function handleEnded() {
  ...
  if (next < window.setlist.length) {
    ...
    audio.src = '/Vinyl Setlist/' + window.setlist[window.currentIndex];
    audio.load();
    ...
    if (typeof window.stopDeck === 'function') window.stopDeck();
    window.currentIndex = 0;
    audio.src = '/Vinyl Setlist/' + window.setlist[0];
    audio.load();
...
loadOkBtn.addEventListener('click', async () => {
  ...
  if (!selected) return;
  await loadSetlistIntoVinyl(selected);
  hide(overlayLoad);
});
...
if (albumNode) {
    ...
    const albumIdAttr = albumNode.getAttribute('data-album-id');

    // ⭐ Saved setlist cover (loaded setlist)
    if (window.currentMode === "setlist" && window.currentSetlistFile) {

      // Hide load/delete (correct)
      loadBtn.style.display = 'none';
      deleteBtn.style.display = 'none';

      // Show Setlist Shuffle
      shuffleSetlistBtn.style.display = 'block';

      // Allow editing saved setlist
      if (editSetlistBtn) editSetlistBtn.style.display = 'block';

      // Hide album shuffle
      shuffleAlbumBtn.style.display = 'none';

      return; // ⭐ STOP — do NOT run album logic
    }
...
qs('#loadSetlistBtn').addEventListener('click', (e) => {
  ...
  if (typeof overlayLoad !== 'undefined' && overlayLoad) {
    ...
    console.warn('overlayLoad not found. Ensure Vinyl-setlist-ui1.js is loaded.');
  }
});
...
qs('#deleteSetlistBtn').addEventListener('click', (e) => {
  ...
  if (typeof overlaySetlistDelete !== 'undefined' && overlaySetlistDelete) {
    ...
    console.warn('overlaySetlistDelete not found. Ensure Vinyl-setlist-ui2.js is loaded.');
  }
});
...
window.loadSetlistIntoVinyl = loadSetlistIntoVinyl;
})();
