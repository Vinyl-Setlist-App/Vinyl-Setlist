...
async function loadShuffledTracks(artistId, coverUrl, tracks) {
  ...
  if (coverImg) {
    // FIX: albumName does NOT exist in shuffle mode
    // Use artist fallback cover instead
    ArtistAlbumCover(artistId, null).then(src => {
      coverImg.src = src;
    });
  }
  ...
  if (audio) {
    ...
    if (relPaths.length) {
      audio.src = '/Vinyl Setlist/' + relPaths[0];
      audio.load();
    }
...
// ------------------------------------------------------------
// 2. Context Menu Logic (Updated: No album shuffle, no saved-setlist shuffle)
// ------------------------------------------------------------
document.addEventListener('contextmenu', async (e) => {
  ...
  // ------------------------------------------------------------
  // CASE 1: Right-click on ALBUM COVER
  // ------------------------------------------------------------
  if (albumNode && albumNode.querySelector('img')) {

    // Artist Shuffle only
    const shuffleBtn = document.createElement('button');
    shuffleBtn.textContent = 'Artist Shuffle';
    shuffleBtn.onclick = async () => {
      ...
      try {
        ...
        // Load cover (random color if missing)
        const coverUrl = `/Vinyl Setlist/${encodeURIComponent(artistId)}/${encodeURIComponent(artistId)}.jpg`;
        loadCover(coverUrl);
  ...
  // ------------------------------------------------------------
  // CASE 2: Right-click on EMPTY LAYER
  // ------------------------------------------------------------
  const shuffleBtn = document.createElement('button');
  shuffleBtn.textContent = 'Artist Shuffle';
  shuffleBtn.onclick = async () => {
    ...
    try {
      ...
      await loadShuffledTracks(artistId, coverUrl, allTracks);
      shuffleCover(); // render shuffle cover
    } catch (err) {
      console.error('[Shuffle] Artist shuffle failed:', err);
    }
  };
...
window.generateSetlistShuffle = async function () {
    ...
    try {
        // Load all albums for this artist
        const albums = await fetchJSON(`/api/artists/${encodeURIComponent(artistId)}/albums`);
        const allTracks = [];
        ...
        // ⭐ Detect saved setlist shuffle → save into /public/Setlist/shuffle-setlist.txt
        if (window.currentMode === "setlist" && window.currentSetlistFile) {
            try {
                await postJSON(`/api/setlists/save-shuffle`, {
                    file: "shuffle-setlist.txt",
                    tracks: allTracks.map(t => t.filePath || t)
                });
                console.log("Saved shuffled setlist to /public/Setlist/shuffle-setlist.txt");
            } catch (err) {
                console.error("Failed to save shuffled setlist:", err);
            }
        }

        // Load shuffled tracks into the player
        await loadShuffledTracks(artistId, coverUrl, allTracks);

        // Render shuffle cover
        shuffleCover();
